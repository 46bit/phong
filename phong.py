...
miki_phone_present = False
other_phone_present = False
...
while True:
    ...
    for event in pygame.event.get():
        if event.type == pygame.USEREVENT:
            if miki_phone_present:
                evt = pygame.event.Event(pygame.KEYDOWN, {"key": K_s})
            else:
                evt = pygame.event.Event(pygame.KEYDOWN, {"key": K_w})
            keydown(evt)

            if other_phone_present:
                evt = pygame.event.Event(pygame.KEYDOWN, {"key": K_DOWN})
            else:
                evt = pygame.event.Event(pygame.KEYDOWN, {"key": K_UP})
            keydown(evt)

            # print("USEREVENT")
            # usb_device_count = int(subprocess.run(["bash", "./usb_device_count.sh"], stdout=subprocess.PIPE).stdout)
            # if usb_device_count == 3:
            #     evt = pygame.event.Event(pygame.KEYDOWN, {"key": K_s})
            # else:
            #     evt = pygame.event.Event(pygame.KEYDOWN, {"key": K_w})
            # print("KEYDOWN", evt)
